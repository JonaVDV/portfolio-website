...
import { createContext, getContext, setContext, tick } from "svelte";
import { SvelteMap, SvelteSet } from "svelte/reactivity";
import { computeCommandScore } from "./search.js";

export const CommandContext = createContext<CommandState>();

const COMMAND_GROUP_KEY = "command-palette-group";
export function getCommandGroupContext() {
  return getContext<string | undefined>(COMMAND_GROUP_KEY);
}
export function setCommandGroupContext(value: string | undefined) {
  setContext(COMMAND_GROUP_KEY, value);
}
...
export class CommandState {
  static create() {
    ...
    const [, setCommandContext] = CommandContext;
  ...
  filteredResults = $derived.by(() => {
    ...
    const sortedIds = Array.from(scores.keys()).sort((a, b) => {
      ...
      const gScoreA = itemA.groupId ? (groupMaxScores.get(itemA.groupId) ?? 0) : scores.get(a) ?? 0;
      const gScoreB = itemB.groupId ? (groupMaxScores.get(itemB.groupId) ?? 0) : scores.get(b) ?? 0;
      ...
      if (itemA.groupId !== itemB.groupId) {
        const firstIdxA = itemA.groupId
          ? this.#getGroupFirstIdx(itemA.groupId)
          : itemA.registrationIndex;
        const firstIdxB = itemB.groupId
          ? this.#getGroupFirstIdx(itemB.groupId)
          : itemB.registrationIndex;
    ...
    const sortedGroupIds = Array.from(activeGroups).sort((a, b) => {
      ...
      return this.#getGroupFirstIdx(a) - this.#getGroupFirstIdx(b);
    });
  ...
  registerItem(id: string, value: string, keywords: string[] = [], groupId?: string) {
    ...
    return () => {
      ...
      if (this.selectedIdState === id) {
        ...
        void tick().then(() => this.selectFirstAvailable());
      }
    };
  }

  #getGroupFirstIdx(groupId: string): number {
  ...
  onKeyDown(e: KeyboardEvent) {
    switch (e.key) {
      ...
      case "Enter": {
        const selectedEl = this.listNode?.querySelector('[data-selected="true"]') as HTMLElement;
        selectedEl?.click();
        break;
      }
    }
  ...
  private async scrollSelectedIntoView() {
    await tick();
    const selectedEl = this.listNode?.querySelector('[data-selected="true"]');
