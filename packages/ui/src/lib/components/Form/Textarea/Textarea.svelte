...
<style>
	textarea {
		...
		--_textarea-background: var(
			--textarea-background,
			var(--input-background, var(--clr-surface-000, #fff))
		);
		...
		&:focus-visible {
			outline: var(--_textarea-focus-outline-width, 1px) solid
				var(--_textarea-focus-outline-color, var(--clr-brand-400, oklch(0.511 0.246 290)));
			outline-offset: 2px;
		...
		&:disabled {
			--_textarea-background: var(
				...
				var(--input-background-disabled, oklch(from var(--_textarea-background) calc(l - 0.03) c h))
			);
			cursor: not-allowed;
			opacity: 0.6;
		}
		...
		&[aria-invalid='true'] {
			--_textarea-border-invalid-color: var(
				--textarea-border-invalid-color,
				var(--input-border-invalid-color, oklch(0.55 0.22 25))
			);
			--_textarea-border-color: var(--_textarea-border-invalid-color);
			--_textarea-focus-outline-color: var(
				--textarea-focus-outline-invalid-color,
				var(--input-focus-outline-invalid-color, var(--_textarea-border-invalid-color))
			);
		}

		&:user-valid {
			--_textarea-border-color: var(
				--textarea-border-valid-color,
				var(--input-border-valid-color, oklch(0.55 0.17 145))
			);
		}
	}
</style>
